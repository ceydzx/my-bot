...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'get') {
    ...
    if (!url) {
      return message.reply('âŒ **Error:** Pakilagay ang URL. Example: `.get https://api.roblox.com`');
    }

    // Step 1: Notify in server channel
    const statusMsg = await message.reply('â³ **Fetching... sent to your DMs**');

    // Step 2: Try DMing the user first to verify DMs are open
    try {
      // Test sending a DM or perform request first
        ...
        if (result.length > 2000) {
          ...
          if (result.length > 6000) {
            ...
            await message.author.send({
              content: `âœ… **Result for:** `${url}` (Attached as .txt because content length is ${result.length} chars):`,
              files: [attachment]
            });
          } else {
            // Split into multiple chunks of ~1900 chars
            const chunks = result.match(/[\s\S]{1,1900}/g) || [result];
            await message.author.send(`âœ… **Result for:** `${url}` (${chunks.length} parts):`);
          ...
          await message.author.send(`âœ… **Result for:** `${url}`\n```js\n${result}\n```);
        }
      } else {
      ...
      if (err.code === 50007 || err.message?.includes('Cannot send messages')) {
        return statusMsg.edit('âš ï¸ **Naka-OFF ang DMs mo!** Paki-enable muna ang **Allow direct messages from server members** sa User Settings > Privacy & Safety para matanggap ang result.');
      }

      console.error('Unexpected error:', err);
      statusMsg.edit('âŒ **An error occurred while processing your request.**');
    }
  }
});
